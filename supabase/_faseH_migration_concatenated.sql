-- ===== 20260909000000_platform_break_glass.sql =====
...
values ('20260909000000', 'platform_break_glass')
on conflict (version) do nothing;

-- =========================================================================================
-- SEED DO PRIMEIRO PLATFORM ADMIN -- NÃO FAZ PARTE DA MIGRATION, E NÃO DEVE FAZER.
-- =========================================================================================
-- Nenhuma função cria o primeiro SUPER: uma função capaz disso seria uma função capaz de
-- cunhar poder de plataforma a partir de uma conta comum. Rode este bloco UMA VEZ, com
-- consciência do que ele faz. Daqui em diante api.grant_platform_admin (só SUPER) cuida do
-- resto.
--
-- Ele NÃO cria contas: as duas identidades abaixo precisam já existir em auth.users. Se
-- ainda não existirem, rode antes:
--     node --env-file=.env scripts/e2e-phase-h.mjs
-- que as provisiona pela Admin API e depois falha dizendo que faltam no cadastro -- essa
-- falha é esperada na primeira execução.

insert into app.platform_admins (user_id, level)
select u.id, 'SUPER'
  from app.users u
 where u.email = 'e2e-selo-platform-super@example.com'
on conflict (user_id) do update set level = 'SUPER', revoked_at = null;

insert into app.platform_admins (user_id, level)
select u.id, 'SUPPORT'
  from app.users u
 where u.email = 'e2e-selo-platform-support@example.com'
on conflict (user_id) do update set level = 'SUPPORT', revoked_at = null;

select u.email, pa.level, pa.revoked_at
  from app.platform_admins pa join app.users u on u.id = pa.user_id;
