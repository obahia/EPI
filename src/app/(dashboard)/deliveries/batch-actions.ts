...
export async function createDeliveryBatch(
  ...
  if (!parsed.success) {
    return { error: parsed.error.issues[0]?.message ?? "Dados inválidos.", links: null, batchId: null };
  }

  const epiIds = formData.getAll("epiId");
  const quantities = formData.getAll("quantity");
  if (epiIds.length === 0) {
    return { error: "Selecione pelo menos um EPI.", links: null, batchId: null };
  }
  ...
  for (let i = 0; i < epiIds.length; i++) {
    ...
    if (!parsedItem.success) {
      return { error: "Selecione um EPI e uma quantidade válida em cada item.", links: null, batchId: null };
    }
  ...
  if (employeeIds.length === 0) {
    return { error: "Selecione pelo menos um funcionário.", links: null, batchId: null };
  }
  if (employeeIds.length > 20000) {
    return { error: "Máximo de 20.000 funcionários por lote.", links: null, batchId: null };
  }
  ...
  const { data, error } = await supabase
    ...
    .rpc("create_delivery_batch", {
      ...
      p_epi_items: items,
      p_confirmations: tokensByEmployee.map((t) => ({ employee_id: t.employeeId, token_hash_b64: t.tokenHashB64 })),
  ...
  if (error || !data) {
    return { error: describeRpcError(error!, "Não foi possível criar o lote."), links: null, batchId: null };
  }
  ...
  return {
    ...
    links: tokensByEmployee.map((t) => ({ employeeFullName: t.employeeFullName, path: `/e/${t.token}` })),
  };
}
...
export async function resendBatchPending(_prevState: ResendBatchState, formData: FormData): Promise<ResendBatchState> {
  ...
  if (!parsed.success) {
    return { error: "Dados inválidos.", links: null };
  }
  ...
  if (listError || !deliveries) {
    return { error: "Não foi possível carregar as entregas do lote.", links: null };
  }
  ...
  const { data, error } = await supabase
    ...
    .rpc("resend_batch_pending", {
      ...
      p_confirmations: tokensByDelivery.map((t) => ({ delivery_id: t.deliveryId, token_hash_b64: t.tokenHashB64 })),
    });

  if (error) {
    return { error: describeRpcError(error, "Não foi possível reenviar o lote."), links: null };
  }
  ...
  return {
    ...
    links: tokensByDelivery
      .filter((t) => resentDeliveryIds.has(t.deliveryId))
      .map((t) => ({ employeeFullName: t.employeeFullName, path: `/e/${t.token}` })),
  };
}
