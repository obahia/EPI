...
export default async function StockEntradaPage({
  searchParams,
}: {
  ...
  // Same "one getEpiVariants call per active EPI, folded into a map" pattern as
  // deliveries/new/page.tsx -- see that page's own comment.
  ...
  return (
    <main className="flex flex-1 flex-col gap-6 p-4 md:p-8">
      <h1 className="font-heading text-4xl font-extrabold tracking-tight">{t.stock.entradaTitle}</h1>
      <p className="text-sm text-muted-foreground">{company.legalName}</p>

      <Card className="max-w-lg">
        <CardHeader>
          <CardTitle>{t.stock.entradaTitle}</CardTitle>
        </CardHeader>
        <CardContent className="flex flex-col gap-4">
          <p className="text-sm text-muted-foreground">{t.stock.entradaHint}</p>
          <EntradaForm
